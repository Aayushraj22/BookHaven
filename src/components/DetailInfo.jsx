...
function DetailInfo() {
  ...
  return (
    <>
    <div className='p-2 dark:bg-stone-950 dark:text-stone-300 h-full px-4'>
      ...
      <div className='flex p-2 px-4 '>
        ...
        <div className='flex flex-col  lg:text-sm w-2/5 justify-center gap-1  capitalize text-sm'>
          <p>Author : <span className=' font-semibold text-stone-900 dark:text-stone-400'>
              {authors?.map(author => <Link key={author._id} to={`/author/${author?._id}`} className='hover:text-blue-400'>{author?.name}</Link>)}
            </span>
          </p>
